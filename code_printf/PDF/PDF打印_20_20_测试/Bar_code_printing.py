...
class SerailUI(wx.Frame):
    ...
    def bmp_to_pdf(self,id,key,x,y):
        ...
        self.f_pdf.setFont("Courier",6*2)
        ...
        if x==0:
            self.f_pdf.drawImage('.\\picture\\' + id + '.bmp', 14 + 2 * 0, self.A4_y - (65*2 * (x + 1)), code_size_x,code_size_y)
            self.f_pdf.drawString(20 + 77 * 0, self.A4_y - 46 - (65 * 2 * (x)), str(id))
        else:
            #self.f_pdf.drawImage('.\\picture\\' + id + '.bmp', 14 + 2 * 0, self.A4_y - (55*2 * (x + 1)), code_size_x,code_size_y)
            #self.f_pdf.drawString(20 + 77 * 0, self.A4_y - 40 - (65 * 2 * (x)), str(id))
            print
    ...
    def button_handle_onprinter(self):
        ...
        try:
            ...
            '''
            tscdll.sendcommand("DIRECTION 1")
            tscdll.windowsfont(35, 19, 20, 0, 0, 0, "Arial", "" + qccode_list['deviceid'])
            ...
            tscdll.sendcommand("PUTBMP 25,24,\"PR.BMP\"")
            tscdll.printlabel("1", "1")
            tscdll.clearbuffer()
            tscdll.closeport()
        except Exception:
...
if __name__=='__main__':
    app = wx.App()
    frame = SerailUI(u"20_20测试打印工具",(100,150),(960,700))
    frame.Show(True)
    app.MainLoop()
